...
function filtarAuto() {
    const resultado = autos.filter(filtarMarca).filter( filtrarYear).filter(filtrarMinimo).filter(filtrarMaximo).filter(filtrarPuertas).filter(filtrarTransmicion).filter(filtrarColor)
...
// se usa parseInt() para transformar year de string a int